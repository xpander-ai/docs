...
first_agent.memory.init_messages(
    input=first_agent.execution.input_message,
    instructions=first_agent.instructions,
...
# Initialize next agent with context from first agent
next_agent.add_task(input=f"Continue processing: {transition_context}")
next_agent.memory.init_messages(
    input=next_agent.execution.input_message,
    instructions=next_agent.instructions,
    llm_provider=LLMProvider.OPEN_AI
)
...
for msg in first_agent.messages[-5:]:  # Last 5 messages for context
    if msg["role"] != "system":  # Skip system messages
        next_agent.memory.add_message(msg)

# Run next agent execution
# ...execution loop for next agent...
```
