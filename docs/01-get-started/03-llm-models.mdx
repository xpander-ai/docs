...
1. Create a task to initialize memory:
```python
# Creating a task automatically initializes the agent's memory
agent.add_task("Your task description here")
```
...
# Add task - this automatically initializes messages
...
while not agent.is_finished():
    ...
    # Add raw response directly to messages
...
- The messages structure is consistent across all providers
