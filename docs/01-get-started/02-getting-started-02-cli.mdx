...
If you encounter issues during installation or configuration, here are solutions to common problems:
...
**On macOS/Linux:**
```bash
# Option 1: Use sudo (not recommended for npm packages)
...
For persistent proxy issues, you may also need to set environment variables:
```bash
...
If `xpander --version` doesn't work after installation:
...
4. **Try running with npx:**
   ```bash
   npx xpander-cli --version
   ```

If issues persist, check that your Node.js version is compatible (Node.js 16+ recommended).
