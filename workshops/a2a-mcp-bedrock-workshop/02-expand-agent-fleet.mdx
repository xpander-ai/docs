...
description: "Create a second coding agent with a different LLM model and custom instructions"
icon: "users"
---

In this module, you'll expand your agent fleet by creating a second coding agent that uses the Llama model rather than Claude. This will give you experience working with multiple agents and different LLM configurations.

🚀 You'll duplicate your existing agent setup, customize it with new instructions, configure it to use Llama, and deploy it to the cloud. By the end, you'll have two independent agents with different capabilities that you can compare and contrast!
...
```bash
# Navigate back to your workshop directory if needed
cd ~/workshop
...
Update the requirement.txt to the following
...
Edit the `agent_instructions.json` file to give your new agent a different personality and focus (or ask cursor):
...
──────────────────────────────────────────────────
Name:     dev-agent-2
ID:       24fcf4ff-de9a-4a5e-94e0-fdd1b5a05428
Icon:     🚀
Model:    openai/gpt-4o
──────────────────────────────────────────────────
...
Next, you need to edit the `coding_agent.py`

Replace the model to the meta llama 4 model
