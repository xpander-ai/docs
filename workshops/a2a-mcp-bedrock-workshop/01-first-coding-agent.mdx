...
In this module, you'll build your first autonomous developer agent using xpander.ai's framework. This agent will be able to understand and manipulate code, helping you automate development tasks.
...
```bash
# Ask Cursor for help with instructions
```
...
To fix this and enable proper responses, modify the on_execution_request function in xpander_handler.py (you can ask Cursor for help or make the change yourself):
...
Stop the current process and restart it with `python xpander_handler.py`.

After making this change, you should be able to send new messages in the Tester tab, and this time, the agent will respond properly!
...
Now that you've built your first developer agent, you're ready to expand your agent fleet in the [next module](/workshops/a2a-mcp-bedrock-workshop/02-expand-agent-fleet).