...
```bash new-agent.sh
xpander agent new --name "stateful-agent" --framework agno --folder .
```
...
```bash invoke-agent.sh
xpander agent invoke "stateful-agent" "hi"
```
...
<Tip>
Access your PostgreSQL database with `myAgent.get_db().db_url` or view all messages using `get_messages_for_session()`.
</Tip>
...
async def my_agent_handler(task: Task):
    ...
    # Get files and images from task for comprehensive processing
