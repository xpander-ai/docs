...
- **Self-hosted API deployment support:** Enabled running the API as a self-hosted container with supporting metadata and environment settings endpoints and execution history operations.
- **Activity timeline with AI Insights:** Introduced a unified, thread-based activity timeline with an AI Insights evaluation flow and optional reasoning-effort controls for supported models.

## Improvements:

- **Workbench planning configuration:** Added consolidated UI controls for planning, reasoning toolkit toggles, and advanced tool-call configuration, including checklist enforcement.
...
- **Deployment and visibility fixes:** Fixed agent deployment error propagation and enforced agent visibility filtering to prevent exposure of other users’ personal agents.
- **MCP tooling fixes:** Corrected MCP tool-selection modal behavior and hid unsupported selections for OAuth2.
- **Agent execution stability:** Enforced consistent inactive-agent handling and added automatic Agno schema migrations on agent database connections.
