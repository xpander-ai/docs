...
<Update label="2025-12-05" description="Agent Activity Threads, Advanced Reasoning Tools, Expanded LLM Providers, and UI Enhancements">

## New Features:

- Introduced agent activity threads API and models, enabling detailed observation of agent execution with new endpoints and Redis-backed storage.
- Added Advanced Reasoning tools toggle and tool call compression settings for Agno agents, with full UI and database support.
- Expanded LLM provider support to include Google AI Studio, Helicone, OpenRouter, and Nebius, with enhanced configuration and deployment options.
- Enabled broader agent framework selection and per-framework feature gating in the frontend, including new icons and UI controls.

## Improvements:

- Enabled reasoning tools and tool call compression by default for Agno agents across backend and frontend.
- Enhanced agent settings UI, including a searchable LLM modal, improved select components, and conditional tab visibility based on user permissions.
- Optimized streaming, task event handling, and performance in agent pipelines and frontend.
- Refined documentation, service descriptions, and default settings for agent creation and configuration.

## Bug Fixes:

- Fixed chat textarea responsiveness and iOS/Safari input height in the Web UI.
- Corrected framework name and icon display in agent settings.
- Resolved OAuth2 flow issues and improved error event payload consistency.
- Addressed task finish signaling, error handling, and terminal state detection in agent executions.
