...
<Update label="2025-12-28" description="API trigger enhancements, new agent tools, PDF layout options, and reliability fixes">
...
- **Deploy comparison before deploy:** Added a pre-deploy comparison flow that shows staged vs deployed changes (added/removed/updated items and configuration diffs) before confirming deployment.
- **Web search and text-to-speech tools:** Added built-in web search and text-to-speech capabilities (including voice listing), with UI icons and lifecycle pairing to keep related tools in sync.
- **Markdown-to-PDF layout modes:** Added multiple PDF layout modes (e.g., whitepaper, invoice, order form, letter), with optional logo, header/footer text, and title support.
...
- **Planning tooling standardization:** Standardized planning tool identifiers to snake_case and introduced multi-complete support for plan tasks.
...
- **MCP OAuth and token flow stability:** Improved MCP OAuth discovery and tightened token exchange validation to prevent invalid credential paths.
- **CI preflight robustness:** Prevented preflight checks from failing on deleted Python files.
