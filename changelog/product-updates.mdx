...
<Update label="2025-12-14" description="Agent import/export improvements, external agent types, A2A protocol updates, and connection flow enhancements">

## New Features:

- **Agent import/export with knowledge base content:** Agents can now be exported and imported with optional inclusion of associated knowledge bases and document content.
- **External agent types (A2A and Curl):** Added new agent types to support calling external agents/services with standardized connectivity details and authentication.
- **A2A protocol (JSON-RPC) support:** Updated the A2A service to use Agent2Agent protocol endpoints (agent card + JSON-RPC methods for sending messages, getting task status, and canceling tasks), including authenticated access.
- **Nano Banana image generation API:** Added an image generation endpoint powered by Google GenAI that uploads generated images to S3 and returns a presigned URL.
- **Unified “Create New Connection” flow:** Introduced a new end-to-end UI for creating and editing connections with improved validation and support for multiple auth methods (API key, OAuth2, AWS).
...
- **Agent activity migration and evaluation:** Improved activity view reliability by migrating legacy sessions into threads on demand and adding a thread evaluation endpoint with caching.
- **MCP and OAuth scope handling:** Enhanced MCP deployment enablement and OAuth scope handling (including additional scopes and refresh-token support).
...
- **Billing and metrics reliability:** Fixed billing metrics reporting and routed usage reporting to a stable dummy agent to improve consistency.
