...
- **Async Redis Migration:** Migrated all agent services to use an asynchronous Redis client, improving throughput and reducing event-loop blocking.
- **Background Task Offloading:** Non-critical cache writes and housekeeping tasks are now handled asynchronously using FastAPI BackgroundTasks, enhancing responsiveness.
- **PII-Safe Metrics and Redaction:** Enhanced reporting by redacting sensitive execution data in metrics and logs, and improved PII handling in agent instructions.
...
- **Basic Auth and Secrets Handling:** Fixed issues with Basic Auth formatting, header propagation, and AWS Secrets Manager integration in AI Gateway and API Caller.
- **Agent Execution Upsert:** Addressed missing-key errors by upserting agent executions on task updates, stabilizing event-driven flows.
...
- **Task Thread Error Handling:** Improved error semantics for task threads and SDK user mapping, ensuring clear API responses and robust user serialization.
