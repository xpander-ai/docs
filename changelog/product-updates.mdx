...
- **Agent Description Field:** Introduced editable agent descriptions in the Workbench and auto-generation of agent descriptions using OpenAI for consistent UX.
- **Built-in Tools Management:** Added toggles for enabling/disabling built-in tools, including a Sleep tool, directly in agent settings.
- **OIDC Redirect Metadata:** Enhanced OIDC login flow to support redirect URLs and propagate metadata through authentication callbacks.
- **Async Redis and Background Tasks:** Migrated platform services to async Redis clients and offloaded non-critical work to background tasks for improved throughput and reliability.

## Improvements:

- **Async Agent Execution and Scheduler:** Refactored agent execution and scheduling to use pure async models, stabilizing event handling and status polling.
- **Agent Activity and Thread Export:** Improved agent activity thread export with deduplication and JSON-safe serialization for downstream consumers.
- **Environment Management:** Unified environment handling and default logic across the UI, with clearer badges, sorting, and admin actions.
...
- **Token Metrics:** Exposed input and output LLM token breakdowns in agent activity threads and sessions for improved monitoring and billing.
...
- **Agent Execution and Error Handling:** Fixed race conditions and error propagation in agent execution, including PII error handling and synthetic thread fallback.
- **Tool-Call Reporting and UI:** Stabilized tool-call reporting, ensured error events are always yielded, and improved UI rendering for tool-call steps.
