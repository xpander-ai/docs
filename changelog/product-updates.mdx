...
- **Removed SSE Transport for MCP:** Deprecated the Server-Sent Events (SSE) transport for the Model Context Protocol (MCP), consolidating to a single HTTP-based server for improved simplicity and alignment with current agent communication models.
- **OAuth2 CLI Login Flow:** Enabled OAuth2-based CLI authentication with callback redirects, streamlining developer experience for CLI integrations.

## Improvements:

- **Header-Based Authentication and Secrets Handling:** Tightened header-based authentication and secrets usage in the AI Gateway proxy, including improved handling of Basic Auth and secrets manager integration.
- **Agent Execution Upsert:** Enhanced agent execution state persistence to prevent missing-key errors during task updates, improving reliability in event-driven flows.
- **LLM Token Metrics:** Exposed input and output LLM token breakdowns in agent activity threads and sessions for improved monitoring and billing.
- **Documentation and UI Consistency:** Updated documentation and minor UI elements for clarity and normalization across services.

## Bug Fixes:

- **Basic Auth and Secrets Manager:** Fixed issues with Basic Auth formatting and secure credential resolution via AWS Secrets Manager, ensuring consistent behavior across gateway and API caller services.
- **Agent Execution Error Handling:** Improved error handling for agent execution flows, including upsert logic and error event emission.
- **General UI and Metrics:** Addressed UI bugs, metrics tab rendering, and ensured accurate input/output token counts in activity views.
...
- **xpander-mono**
  - Deprecated the Server-Sent Events (SSE) transport for the Model Context Protocol (MCP), consolidating all MCP communication to a single HTTP-based server. This simplifies the integration and reduces resource usage.

- **frontend**
  - Added OAuth2 CLI login flow with callback redirect, enabling seamless OAuth2-based CLI authentication for developers.

## Improvements

- **xpander-mono**
  - Enhanced AI Gateway proxy by consolidating Basic Auth handling, improving secrets management, and adding request payload flexibility for development utilities.
  - Improved outbound connector requests to consistently handle Basic Auth formatting and securely resolve credentials via AWS Secrets Manager, with optional role assumption support.
  - Exposed per-thread LLM token breakdown (input/output tokens) in agent activity for improved monitoring and billing.
  - Increased Redis cache TTL for billing quota validation from 3 to 10 minutes to reduce repeated quota checks and external API load.
  - Exposed tool-call reasoning metadata (title and description) in agent runtime and UI to improve reviewability of tool invocations.

- **frontend**
  - Split thread activity metrics into input and output tokens and adjusted the layout for improved clarity in the Threads activity UI.
  - Refined agent metrics, enabled opening agent creation via URL, and standardized UX labels for improved dashboard clarity.
  - Added a built-in Sleep tool to agent settings and improved the chat WebUI, including support for self-hosted chat URLs and enhanced progress visibility in thread logs.
  - Enabled deep-linking from the Tasks list to the monitor tab and auto-scroll to the relevant thread for faster triage and debugging.
  - Improved OAuth2 login flow with API key selection and authorization UI for CLI authentication.

## Bug Fixes

- **xpander-mono**
  - Fixed agent execution state persistence to prevent missing-key errors during task updates by upserting agent executions when records are missing.
  - Stabilized tool-call reporting, generalist instructions, and agent error flow for improved reliability and clarity.
  - Addressed bugs impacting agent execution flow and Chainlit UI, tightened PII/instructions handling, and ensured consistent tool-call step rendering.
  - Improved task thread error handling and xpander SDK user mapping for more explicit API error semantics.
  - Fixed agno setting and version bump.

- **frontend**
  - Fixed UI issues including text changes in agno settings, default masking of detected PII, and metrics tab display.
  - Ensured safe redirect handling for WebUI authentication, only allowing trusted destinations and correct session token forwarding.
  - Addressed race conditions in the "Just a minute" modal and ensured built-in tools are open by default.
  - Fixed chat WebUI behaviors and added support for toggling the Sleep tool in agent settings.
  - Fixed small UI issues including footer display, version selection, and upload modal.
