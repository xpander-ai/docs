...
<Update label="2025-11-23" description="Generalist Agent Revamp, Timezone Awareness, and UI/UX Enhancements">
...
- **Agent Card Endpoint:** Exposed a standardized agent card endpoint for improved agent discoverability and integration.
- **Timezone Awareness:** Chat sessions now capture and sync the user's IANA timezone from the browser to the backend, enabling personalized context and improved downstream processing.
- **User Details Propagation:** Webhook and agent invocation pipelines now accept and propagate user identity details for richer personalization and auditing.
- **MCP Client Integration:** The MCP client has been consolidated into the Agents service, providing unified endpoints for listing and invoking remote MCP tools with robust transport detection and retries.

## Improvements:

- **Reasoning Pipeline and Caching:** Enhanced the reasoning extraction and caching logic for agent executions, with dynamic Redis TTLs and improved error handling.
- **UI/UX Enhancements:** 
  - Overhauled the Chainlit WebUI with stabilized task sync indicators, improved task grouping, and live reasoning display.
  - Refined logs panel with duration indicators, operation icons, and improved readability.
  - Updated agent branding and instruction phrasing for clarity and consistency.
  - Improved file extraction and rendering for images and attachments in chat threads.
  - Enhanced connector management, including connection switching, multi-delete, and version upgrades.
- **Metrics and Privacy:** Redacted sensitive user input in metrics payloads and improved logging for billing and entitlement usage.
- **Documentation:** Expanded and updated service READMEs, including detailed features, API endpoints, and configuration guidance.
...
- **Timezone Sync Reliability:** Improved reliability of timezone synchronization between the WebUI and backend, with fallback mechanisms and session validation.
...
- **Billing Quota Calculation:** Corrected entitlement-based usage calculations for billing quotas and sunsetted the MCP service across workflows.
