...
description: "Learn how to integrate AI agents with AWS Cost Exporer MCP Server, Notion, and built-in tools with xpander.ai"
icon: "clipboard"
---
## Introduction

Automating tasks like report and insight generation has never been easier, thanks to advancements in AI.

We can now integrate AI systems with internal resources, such as databases, personal calendars, and communication logs, using concepts like function calling and the Model Context Protocol (MCP).

This allows AI to automate tasks fully personalized to our needs, whether it's summarizing weekly performance reports, generating client-specific presentations, or drafting tailored responses to customer inquiries.
...
- **Notion interface**: to transform the analysis into a final report based on a provided template in Notion. You can view the example template on [this Notion page](https://www.notion.so/AWS-Usage-Cost-Summary-21fad78b444680d2b915dc94ec6d27b6?source=copy_link) and copy it into your own workspace.
...
![img_4](/images/tutorial-5/4.png)

### Create an xpander configuration file

Add your xpander configuration—agent ID, organization ID, and xpander API key—into a file named `xpander_config.json` as follows:
...
To locate your **Agent ID**, navigate to the **"Connect"** tab in the Agent configuration—you’ll find the Agent ID under the **SDK** section.
...
```python
# Inside a Python file called agno_agent_with_aws_mcp.py
...
from pathlib import Path
from dotenv import load_dotenv
from agno_agent_with_aws_mcp import AgnoAgent
