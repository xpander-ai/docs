---
title: "Agent Versioning"
description: "Track, rollback, and manage agent versions with full history and audit trail"
icon: "code-branch"
---

Agent Versioning provides complete version control for your agents, including configuration, prompts, tools, and code. Track changes, compare versions, and rollback instantly when needed.

## Overview

<CardGroup cols={2}>
  <Card title="Version History" icon="clock-rotate-left">
    Complete history of all agent changes with timestamps and authors
  </Card>
  <Card title="Instant Rollback" icon="rotate-left">
    Revert to any previous version with one click
  </Card>
  <Card title="Compare Versions" icon="code-compare">
    Side-by-side diff of configuration changes between versions
  </Card>
  <Card title="Audit Trail" icon="list-check">
    Full audit log of who changed what and when
  </Card>
</CardGroup>

## What Gets Versioned

Every aspect of your agent is tracked:

| Component | Tracked Changes |
|-----------|----------------|
| System Prompt | Content, parameters, model settings |
| Tools | Added, removed, or reconfigured tools |
| Configuration | Model selection, temperature, tokens |
| Knowledge Base | Document additions and removals |
| Connections | OAuth and API key changes |

## Version Management

### Creating Versions

Versions are created automatically when you:

- Save changes in the Workbench
- Deploy via CLI or SDK
- Import an agent configuration

```bash
# Deploy creates a new version
xpander deploy --message "Added GitHub integration"
```

### Viewing Version History

Access version history in the Workbench under **Agent Settings > Versions**:

```
v12 (current) - 2 hours ago - "Updated system prompt for clarity"
v11 - 1 day ago - "Added Slack connector"
v10 - 3 days ago - "Increased max tokens to 4096"
v9 - 1 week ago - "Initial production deployment"
```

### Comparing Versions

Compare any two versions to see exactly what changed:

```diff
System Prompt:
- You are a helpful assistant.
+ You are a helpful DevOps assistant specializing in Kubernetes.

Tools:
+ github-issues-manager (added)
+ github-actions-manager (added)

Configuration:
- max_tokens: 2048
+ max_tokens: 4096
```

### Rolling Back

Rollback to any previous version instantly:

```bash
# Via CLI
xpander rollback --version v10

# Or via SDK
from xpander_sdk import Agents

agents = Agents()
agents.rollback(agent_id="agent-123", version="v10")
```

<Warning>
Rollback creates a new version with the old configuration. The version history is never deleted.
</Warning>

## Version Pinning

Pin a specific version for production stability:

```python
from xpander_sdk import Backend

# Always use version v10 regardless of newer deployments
backend = Backend(agent_version="v10")
```

This is useful for:
- A/B testing different agent versions
- Gradual rollouts
- Debugging production issues

## Export and Import

Export any version as a portable configuration:

```bash
# Export specific version
xpander export --version v10 --output agent-v10.json

# Import to create new agent or restore
xpander import --file agent-v10.json
```

### Export Format

```json
{
  "version": "v10",
  "exported_at": "2024-01-15T10:30:00Z",
  "agent": {
    "name": "DevOps Assistant",
    "system_prompt": "...",
    "model": "gpt-4",
    "tools": ["github-issues-manager"],
    "configuration": {
      "temperature": 0.7,
      "max_tokens": 4096
    }
  }
}
```

## Branch Deployments

Deploy different versions to different environments:

| Environment | Version | Purpose |
|-------------|---------|---------|
| Production | v10 (pinned) | Stable, tested |
| Staging | v12 (latest) | Pre-production testing |
| Development | HEAD | Active development |

## Related

<CardGroup cols={2}>
  <Card title="Agent Import/Export" icon="file-export" href="/api-reference/agents/import-export">
    API reference for import/export operations
  </Card>
  <Card title="Deployment" icon="server" href="/user-guide/run/self-hosted-deployment">
    Deploy agents to production
  </Card>
</CardGroup>
