...
2. Fetch config with `Agents().aget()`
3. Initialize your framework with xpander's config
...
When you fetch an agent with `Agents().aget()`, you get access to:

| Property                    | Description                       |
| --------------------------- | --------------------------------- |
| `model_name`                | Configured model (e.g., "gpt-4o") |
| `model_provider`            | Provider (e.g., "openai")         |
| `instructions.full`         | Complete system prompt            |
| `instructions.instructions` | Core instructions                 |
| `instructions.description`  | Agent description                 |
| `tools.functions`           | Generic tool functions            |
| `strands_tools`             | Strands-compatible tools          |
| `openai_agents_sdk_tools`   | OpenAI Agents SDK tools           |
| `sanitized_name`            | URL-safe agent name               |
| `aget_db()`                 | Get PostgreSQL connection         |
...
async def my_agent_handler(task: Task):
    ...
    agno_agent = Agent(
        db=await xpander_agent.aget_db(),
        session_id=task.id,
        add_history_to_context=True,
        model=OpenAIChat(id=xpander_agent.model_name, api_key="{YOUR_LLM_KEY}"),
        ...
        additional_context=f"User details: {task.input.user.model_dump_json()}" if task.input.user else None
    )
...
<Note>
For self-hosted deployment on your own Kubernetes cluster, see the [Self-Hosted Deployment Guide](/user-guide/self-hosted-deployment).
</Note>
