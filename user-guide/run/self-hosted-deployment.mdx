...
xpander.ai can be deployed on your own infrastructure using Helm charts, giving you full control over your AI agent platform.

<CardGroup cols={2}>
  <Card title="5-Minute Quick Deploy" icon="bolt" href="/Examples/5-minute-wins/k8s-self-deploy">
    Get started fast with step-by-step K8s deployment tutorial
  </Card>
  <Card title="Detailed Configuration" icon="sliders">
    Complete reference guide (continue reading below)
  </Card>
</CardGroup>
...
**[On-Premises Architecture Guide](https://pages.xpander.ai/on-premises-architecture)**
...
curl http://agent-controller.my-company.com/health
...
## Using Self-Hosted Deployment with xpander SDK

To use your self-hosted deployment with the xpander SDK, configure the `base_url` to point to your Agent Controller endpoint.

<Note>
**API Key**: When using self-hosted deployment, use the **Agent Controller API key** generated during Helm installation, not your xpander.ai cloud API key.
</Note>
...
For more examples, see the [Self-Hosted Configuration Guide](https://docs.xpander.ai/api-reference/configuration/self-hosted).
</Note>
...
- **Configure Agents**: Set up your AI agents using the [Quick Start Guide](/user-guide/index)
- **Connect Slack**: Integrate with Slack using the [Slack Integration Guide](/user-guide/scale/slack)
