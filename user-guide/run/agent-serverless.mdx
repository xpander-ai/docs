...
description: "Deploy agents from Workbench without managing code or infrastructure"
icon: "bolt"
---

Serverless is the default deployment mode when you build agents entirely in **Workbench** without customizing code. Your agent runs on agent-workers (xpander cloud or self-hosted) without you managing containers or code files.
...
2. **Click Deploy**: Agent runs serverlessly on agent-workers
...
<Steps>
  ...
  <Step title="Deploy from Workbench">
    ...
    Your agent automatically runs serverlessly on agent-workers.
