...
```mermaid
graph TB
    Cloud["☁️ xpander.ai Cloud<br/>Metadata • Registrations • Events"]
    ...
    subgraph "Self-Hosted"
        ...
        PG["PostgreSQL<br/>Data & Persistence"]
    end
    
    %% Registration flows (metadata only)
    AC -.->|"metadata & events"| Cloud
    AG -.->|"registers in"| AC
    MCP -.->|"registers in"| AC
    AW -.->|"registers in"| AC
    
    %% Data flows (stays within infrastructure)
    AC <-->|"API calls"| AG
    AC <-->|"state"| Redis
    AC <-->|"data"| PG
    ...
    classDef cloud fill:#e3f2fd,stroke:#1976d2,stroke-width:3px
