...
The `internal_status` field provides custom status tracking with a 255 character limit.
...
async def track_internal_status():
    ...
    # Create task and set initial internal status
    ...
    task.internal_status = "Validating order data"
    await task.asave()
    
    # Continue updating internal status
    task.internal_status = "Processing payments"
    await task.asave()
    
    # Final internal status
    task.internal_status = "Orders processed and shipped"
    await task.asave()
    
    print(f"Task Status: {task.status}")           # e.g., "executing"
    print(f"Internal Status: {task.internal_status}")  # "Orders processed and shipped"
    
    # Validate character limit (255 characters max)
    long_status = "Processing data" * 20  # This would exceed 255 characters
    if len(long_status) > 255:
        task.internal_status = long_status[:255]  # Truncate to fit limit
    else:
        task.internal_status = long_status
