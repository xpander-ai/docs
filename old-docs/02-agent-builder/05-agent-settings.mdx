...
# Get existing agent backend
agent_backend = agents.get("your-agent-id")

# Agent backend provides access to agent configuration
# Agents are managed through the xpander.ai console at https://app.xpander.ai
print(f"Agent Name: {agent_backend.name}")
print(f"Agent ID: {agent_backend.id}")
print(f"Is Active: {agent_backend.is_active}")

# All agent configuration is done through the console
# Visit https://app.xpander.ai to:
